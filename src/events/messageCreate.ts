...
export async function handleMessage(message: Message): Promise<void> {
  ...
  console.log(
    `Message received in target channel from ${message.author.username}: "${message.content}"`
  );

  let scoreData: Score | null = null;
  try {
    // Try to parse the message as a score submission
    scoreData = parseScoreMessage(message);
    if (!scoreData) {
      console.log("No score data found in message");
      return;
    }
  } catch (error) {
  ...
  if (scoreData) {
    ...
    // Check attempts
    const currentAttempts = await getPlayerAttemptsToday(
      scoreData.playerId,
      messageDate
    );
    console.log(
      `Current attempts for ${scoreData.playerName}: ${currentAttempts}`
    );
    ...
    console.log(
      `Score saved for ${scoreData.playerName}: attempt ${attemptNumber}, isFirst: ${isFirst}`
    );
    ...
    try {
      ...
      console.log(
        `Recorded score for ${scoreData.playerName}: ${scoreData.strokes} strokes (Attempt ${attemptNumber}/${MAX_ATTEMPTS})`
      );
    } catch (error) {
...
async function checkTopScoreAndReact(
  message: Message,
  scoreData: Score
): Promise<void> {
  ...
  const isFirstScoreOfDay =
    firstScores.length <= 1 &&
    bestOfThreeScores.length <= 1 &&
    unlimitedScores.length <= 1;
  ...
  if (topCategories.length > 0 && !isFirstScoreOfDay) {
    ...
    await message.reply(
      `Congratulations! This is the lowest score today for ${categoryText}! 🏆`
    );
  }
}
